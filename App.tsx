...
import { 
  ...
  History, Send, Sparkles, ChevronRight, 
  ExternalLink, Facebook, Globe, Youtube, 
  Radio, Loader2
} from 'lucide-react';
import { ChatMessage, PlayerState, GroundingSource } from './types';
...
const CONFIG = {
  ...
  PROGRAMS: [
    { id: '1', time: '12:00', title: 'Sermon du Vendredi (Live)', isActive: true },
    { id: '2', time: '11:00', title: 'Lecture du Coran - Juz 15', isActive: false },
    { id: '3', time: '10:00', title: 'Leçons de Vie Islamique', isActive: false },
    { id: '4', time: '09:00', title: 'Émission Jeunesse', isActive: false },
    { id: '5', time: '07:00', title: 'Invocations du Matin', isActive: false },
  ]
};
...
const App: React.FC = () => {
  ...
  const togglePlay = useCallback(() => {
    ...
    if (playerState === PlayerState.PLAYING || playerState === PlayerState.BUFFERING) {
      ...
      audioRef.current.src = ""; // Clean up the buffer
      audioRef.current.load();
      setPlayerState(PlayerState.PAUSED);
    } else {
      setError(null);
      setPlayerState(PlayerState.BUFFERING);
      audioRef.current.src = `${CONFIG.STREAM_URL}?t=${Date.now()}`;
      audioRef.current.volume = isMuted ? 0 : volume / 100;
      audioRef.current.play().catch(e => {
        console.error("Playback Error:", e);
        setError("Impossible de charger le direct.");
        setPlayerState(PlayerState.PAUSED);
      });
    }
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setChatMessages(prev => [...prev, { 
        role: 'model', 
        text: "La connexion avec l'IA a été interrompue. Veuillez réessayer.", 
        timestamp: new Date() 
      }]);
    } finally {
      setIsTyping(false);
  ...
  return (
    <div className="h-screen w-screen bg-[#022c22] text-[#fcf8f1] flex flex-col md:flex-row overflow-hidden select-none relative">
      <audio 
        ...
        onError={() => setError("Erreur de connexion au flux.")}
        crossOrigin="anonymous"
      />

      {/* Main UI */}
      <main className={`flex-1 flex flex-col items-center justify-start md:justify-center p-4 md:p-8 transition-all duration-700 overflow-y-auto scrollbar-hide ${showChat ? 'md:mr-[400px] lg:mr-[450px]' : ''}`}>
        
        {/* Player Card */}
        <div className="max-w-md w-full glass-panel rounded-[3.5rem] p-8 md:p-12 shadow-2xl relative overflow-hidden my-auto border border-amber-500/20">
          ...
          <div className="relative z-10 text-center mb-10">
            <div className="inline-block relative">
              ...
              <div className="w-56 h-56 md:w-64 md:h-64 rounded-full border-[6px] border-amber-500/30 p-3 overflow-hidden bg-emerald-950/80 shadow-2xl relative transform transition-transform duration-500 hover:scale-105">
                <img 
                  src="https://i.postimg.cc/WzDp64wr/iqra-logo-ok.png" 
                  alt="Radio Iqra" 
                  className={`w-full h-full object-cover rounded-full ${playerState === PlayerState.PLAYING ? 'animate-[spin_30s_linear_infinite]' : ''}`}
                />
                <button 
                  onClick={togglePlay}
              ...
              <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 bg-red-600 px-6 py-1.5 rounded-full border-2 border-amber-500 flex items-center gap-2 shadow-2xl animate-live-badge z-20">
                <div className="w-2 h-2 bg-white rounded-full" />
                <span className="text-[11px] font-black uppercase tracking-[0.2em] text-white">Direct</span>
              </div>
            </div>
            
            <h1 className="mt-12 text-3xl font-black tracking-tighter text-amber-50 drop-shadow-lg uppercase">Radio Iqra BF</h1>
            <p className="text-amber-500 font-bold text-[11px] tracking-[0.5em] mt-2 opacity-90 uppercase">La Voix du Saint Coran</p>
          </div>
          ...
          <div className="flex flex-col gap-8 relative z-10">
            ...
            <div className="flex items-center justify-between gap-6 px-4">
              <button onClick={() => { setIsMuted(!isMuted); if(audioRef.current) audioRef.current.muted = !isMuted; }} className="text-amber-500 hover:text-amber-300 transition-colors p-2">
              ...
              />
              <button onClick={() => navigator.share?.({title: 'Radio Iqra BF', url: window.location.href})} className="text-amber-500 hover:text-amber-300 transition-colors p-2">
            ...
            <div className="bg-amber-500/10 rounded-[2.5rem] p-6 border border-amber-500/30 shadow-inner group transition-all hover:bg-amber-500/20">
              <div className="flex items-center gap-5">
                ...
                <div className="overflow-hidden">
                  <p className="text-[10px] uppercase font-black text-amber-500 mb-1 tracking-widest opacity-80 italic">Direct 96.1 MHz</p>
                  <p className="font-bold text-amber-50 truncate text-xl tracking-tight leading-tight">Programme National</p>
                  <p className="text-[11px] text-amber-200/50 font-bold uppercase tracking-widest">Burkina Faso</p>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* Links Section */}
        {!showChat && (
          <div className="w-full max-w-md space-y-12 animate-in fade-in slide-in-from-bottom-12 duration-1000 pb-24 pt-12">
            <div className="grid grid-cols-3 gap-5 px-2">
              {CONFIG.SOCIAL_LINKS.map((link) => (
                <a key={link.name} href={link.url} target="_blank" rel="noopener noreferrer" 
                  className="flex flex-col items-center gap-3 p-5 rounded-[2.5rem] glass-panel border border-amber-500/10 hover:border-amber-500/60 hover:bg-amber-500 hover:text-[#022c22] transition-all group shadow-2xl">
            ...
            <div className="px-2">
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="font-black text-amber-100 text-[12px] uppercase tracking-[0.4em]">À suivre</h2>
                </div>
                <div className="h-px flex-1 bg-amber-500/20 ml-6" />
              </div>
              <div className="space-y-4">
                {CONFIG.PROGRAMS.map(prog => (
                  <div key={prog.id} className={`flex items-center gap-5 p-5 rounded-[2.2rem] border transition-all duration-300 ${prog.isActive ? 'glass-panel border-amber-500/40 shadow-2xl' : 'bg-emerald-950/40 border-transparent'}`}>
                    <span className="text-[11px] font-black text-amber-500 w-14 text-center bg-amber-500/10 py-1.5 rounded-xl border border-amber-500/20">{prog.time}</span>
                    <div className="flex-1 min-w-0">
                      <p className={`text-base font-bold truncate ${prog.isActive ? 'text-white' : 'text-emerald-100/70'}`}>{prog.title}</p>
                    </div>
                    {prog.isActive && <div className="w-3 h-3 rounded-full bg-amber-400 animate-pulse shadow-[0_0_15px_#d4af37]" />}
                  </div>
                ))}
              </div>
            </div>
      ...
      <aside className={`fixed inset-y-0 right-0 w-full md:w-[400px] lg:w-[450px] bg-[#022c22]/95 backdrop-blur-3xl border-l border-amber-500/20 shadow-2xl transition-transform duration-700 ease-out z-50 flex flex-col ${showChat ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-10 scrollbar-thin">
          {chatMessages.length === 0 && (
            <div className="text-center py-24 space-y-8">
              <Sparkles className="w-20 h-20 text-amber-500/30 mx-auto" />
              <div className="space-y-3">
                <h4 className="font-black uppercase tracking-widest text-lg text-amber-100">Paix et Salut</h4>
                <p className="text-[11px] max-w-[280px] mx-auto font-bold text-amber-200/40 uppercase tracking-widest leading-loose">Posez vos questions sur la foi, les sourates ou la programmation de la radio.</p>
              </div>
            </div>
          )}
                  ...
                  <div className="mt-6 pt-5 border-t border-amber-500/10 flex flex-col gap-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {msg.sources.map((s, idx) => s.web && (
                        <a key={idx} href={s.web.uri} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 bg-emerald-950/80 hover:bg-amber-500 hover:text-[#022c22] px-4 py-2 rounded-full text-[10px] font-black transition-all border border-amber-500/20">
        ...
        <div className="p-10 bg-emerald-950/60 border-t border-amber-500/10 shadow-2xl">
          <form onSubmit={handleSendMessage} className="relative group">
            ...
            <button 
              ...
              className="absolute right-3 top-3 bottom-3 w-14 bg-amber-500 hover:bg-amber-400 disabled:bg-emerald-900/50 disabled:text-amber-900 rounded-2xl transition-all flex items-center justify-center text-[#022c22] shadow-2xl"
            >
              <Send className="w-6 h-6" />
            </button>
          </form>
          <p className="text-[10px] text-center text-amber-800 font-black uppercase mt-6 tracking-[0.5em] opacity-40 italic">Intelligence Spirituelle • Gemini</p>
        </div>
      </aside>

      {/* Floating Sparkle Button for Mobile Chat */}
      {!showChat && (
        <button 
          onClick={() => setShowChat(true)}
          className="md:hidden fixed bottom-8 right-8 w-20 h-20 bg-amber-500 text-[#022c22] rounded-full flex items-center justify-center shadow-[0_20px_50px_rgba(212,175,55,0.4)] z-40 transform hover:scale-110 active:scale-90 transition-all border-4 border-[#022c22]"
        >
